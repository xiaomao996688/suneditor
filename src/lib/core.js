...
export default function (
  ...
  const core = {
    ...
    _cleanStyleRegExp: {
      ...
      format: new _w.RegExp(
        "\\s*[^-a-zA-Z](text-align|margin-left|margin-right|width)\\s*:[^;]+(?!;)*",
        "ig"
      ),
    ...
    _saveButtonStates: function () {
      ...
      const currentButtons = this.context.element._buttonTray.querySelectorAll(
    ...
    _recoverButtonStates: function () {
      if (this.allCommandButtons) {
        ...
        for (
          let i = 0, button, command, oldButton;
    ...
    setControllerPosition: function (controller, referEl, position, addOffset) {
      ...
      const allow = util.hasClass(controller.firstElementChild, "se-arrow")
    ...
    setRange: function (startCon, startOff, endCon, endOff) {
      ...
      if (util.isFormatElement(endCon)) {
        endCon =
          endCon.childNodes[endOff] || endCon.childNodes[endOff - 1] || endCon;
        endOff =
          endOff > 0
            ? endCon.nodeType === 1
              ? 1
    ...
    getSelection: function () {
      const selection =
        this._shadowRoot && this._shadowRoot.getSelection
    ...
    _dupleCheck: function (oNode, parentNode) {
      ...
      (function recursionFunc(ancestor) {
        if (util.isWysiwygDiv(ancestor) || !util.isTextStyleElement(ancestor))
          return;
    ...
    insertNode: function (oNode, afterNode, checkCharCount) {
      ...
      let range = this.getRange();
      let line = util.isListCell(range.commonAncestorContainer)
        ? range.commonAncestorContainer
        : util.getFormatElement(this.getSelectionNode(), null);
      let insertListCell =
        util.isListCell(line) && (util.isListCell(oNode) || util.isList(oNode));
            ...
            if (isSameContainer) {
              ...
              if (util.isListCell(container) && oNode.nodeType === 3) {
                ...
                parentNode = container.appendChild(
                  util.createElement(options.defaultTag)
                );
                afterNode = null;
              } else {
                afterNode = isFormats
      ...
      try {
        ...
        if (dupleNodes.length > 0) {
          for (
            let i = 0, len = dupleNodes.length, d, c, ch, parent;
            i < len;
            i++
          ) {
            d = dupleNodes[i];
    ...
    removeNode: function () {
      ...
      const isEndEdge = core.isEdgePoint(
        range.endContainer,
        range.endOffset,
        "end"
      );
      ...
      if (startCon === endCon && range.collapsed) {
        if (
          startCon.textContent &&
          util.onlyZeroWidthSpace(startCon.textContent.substr(startOff))
        ) {
          return {
            container: startCon,
            offset: startOff,
      ...
      if (
        !util.getFormatElement(container) &&
        !(startCon && startCon.parentNode)
      ) {
        if (endNextEl) {
          container = endNextEl;
    ...
    toggleCodeView: function () {
      ...
      if (this.isReadOnly)
        util.setDisabledButtons(true, this.resizingDisabledButtons);
    ...
    setContents: function (html) {
      ...
      const convertValue =
        html === null || html === undefined
          ? ""
          : this.convertContentsForEditor(html, null, null);
    ...
    setIframeContents: function (ctx) {
      ...
      if (ctx.head)
        this._wd.head.innerHTML = this.options.__allowedScriptTag
          ? ctx.head
          : ctx.head.replace(this.__scriptTagRegExp, "");
      if (ctx.body)
      ...
      if (node.nodeType === 1) {
        ...
        const ch =
          util.getListChildNodes(node, function (current) {
            return (
              util.isSpanWithoutAttr(current) &&
              !util.getParentElement(current, util.isNotCheckingNode)
            );
          }) || [];
        ...
        if (
          ...
          return (
            ...
            defaultTag +
            ">" +
            (util.isSpanWithoutAttr(node) ? node.innerHTML : node.outerHTML) +
            "</" +
            defaultTag +
            ">"
          );
      ...
      if (/span/i.test(name) && !sv && (m.match(/<span\s(.+)/) || [])[1]) {
        ...
        if (size || face || color) {
          sv =
            'style="' +
            (size
              ? "font-size:" + this.util.getNumber(size / 3.333, 1) + "rem;"
              : "") +
              ...
              switch (k) {
                ...
                case "fontSize":
                  ...
                  if (!this._cleanStyleRegExp.fontSizeUnit.test(r[0])) {
                    r[0] = r[0].replace(
                      (r[0].match(/:\s*([^;]+)/) || [])[1],
                      this._convertFontSize.bind(this, options.fontSizeUnit)
                    );
                  }
                  break;
                case "color":
    ...
    _cleanTags: function (lowLevelCheck, m, t) {
      ...
      if (v) {
        for (let i = 0, len = v.length, a; i < len; i++) {
          ...
          a = /^(?:href|src)\s*=\s*('|"|\s)*javascript\s*\:/i.test(v[i].trim())
            ? ""
            : v[i];
          t += (/^\s/.test(a) ? "" : " ") + a;
        }
    ...
    _isFormatData: function (domTree) {
      ...
      for (let i = 0, len = domTree.length, t; i < len; i++) {
        ...
        if (
          ...
          !util.isBreak(t) &&
          !this.__disallowedTagNameRegExp.test(t.nodeName)
        ) {
          requireFormat = true;
          break;
        }
    ...
    cleanHTML: function (html, whitelist, blacklist) {
      html = this._deleteDisallowedTags(
        this._parser.parseFromString(util.htmlCompress(html), "text/html").body
          .innerHTML
      )
        .replace(
          /(<[a-zA-Z0-9\-]+)[^>]*(?=>)/g,
    ...
    convertContentsForEditor: function (contents) {
      contents = this._deleteDisallowedTags(
        this._parser.parseFromString(util.htmlCompress(contents), "text/html")
          .body.innerHTML
      ).replace(
        /(<[a-zA-Z0-9\-]+)[^>]*(?=>)/g,
        this._cleanTags.bind(this, true)
    ...
    _setKeyEffect: function (ignoredList) {
      ...
      for (let key in commandMap) {
        if (ignoredList.indexOf(key) > -1 || !util.hasOwn(commandMap, key))
          continue;
        if (activePlugins.indexOf(key) > -1) {
          plugins[key].active.call(this, null);
        } else if (commandMap.OUTDENT && /^OUTDENT$/i.test(key)) {
          if (!util.isImportantDisabled(commandMap.OUTDENT))
            commandMap.OUTDENT.setAttribute("disabled", true);
        } else if (commandMap.INDENT && /^INDENT$/i.test(key)) {
          if (!util.isImportantDisabled(commandMap.INDENT))
    ...
    _init: function (reload, _initHTML) {
      ...
      this._editorHeightPadding =
        util.getNumber(this.wwComputedStyle.getPropertyValue("padding-top")) +
        util.getNumber(this.wwComputedStyle.getPropertyValue("padding-bottom"));
      ...
      this.__scriptTagRegExp = new wRegExp(
        "<(script)[^>]*>([\\s\\S]*?)<\\/\\1>|<script[^>]*\\/?>",
        "gi"
      );
      this.__disallowedTagsRegExp = new wRegExp(
        "<(" +
          sPrefix +
          "style)[^>]*>([\\s\\S]*?)<\\/\\1>|<(" +
          sPrefix +
      ...
      this.__disallowedTagNameRegExp = new wRegExp(
        "^(" + sPrefix + "meta|link|style|[a-z]+:[a-z]+)$",
        "i"
      );
      this.__allowedScriptRegExp = new wRegExp(
        "^" + (options.__allowedScriptTag ? "script" : "") + "$",
        "i"
      );
      ...
      this._htmlCheckWhitelistRegExp = new wRegExp(
        "^(" +
          getRegList(options._editorTagsWhitelist.replace("|//", ""), "") +
          ")$",
        "i"
      );
      ...
      if (!!_wAttr) {
        for (let k in _wAttr) {
          if (!util.hasOwn(_wAttr, k) || /^on[a-z]+$/i.test(_wAttr[k]))
            continue;
          if (k === "all") {
            allAttr = getRegList(_wAttr[k], defaultAttr);
          } else {
            tagsAttr[k] = new wRegExp(
              "\\s(?:" + getRegList(_wAttr[k], "") + ")" + regEndStr,
              "ig"
            );
          }
        }
      ...
      this._attributesWhitelistRegExp = new wRegExp(
        "\\s(?:" + (allAttr || defaultAttr + "|" + dataAttr) + ")" + regEndStr,
        "ig"
      );
      this._attributesWhitelistRegExp_all_data = new wRegExp(
        "\\s(?:" +
          ((allAttr || defaultAttr) + "|data-[a-z0-9\\-]+") +
          ")" +
          regEndStr,
        "ig"
      );
      ...
      if (!!_bAttr) {
        for (let k in _bAttr) {
          ...
          if (k === "all") {
            ...
            tagsAttr[k] = new wRegExp(
              "\\s(?:" + getRegList(_bAttr[k], "") + ")" + regEndStr,
              "ig"
            );
          }
        }
      ...
      context.element.wysiwyg.innerHTML = reload
        ...
        : this.convertContentsForEditor(
            (typeof _initHTML === "string"
              ? _initHTML
              : /^TEXTAREA$/i.test(context.element.originElement.nodeName)
                ? context.element.originElement.value
                : context.element.originElement.innerHTML) || ""
          );
    },
      ...
      h =
        h === -1
          ? resizeObserverEntry.borderBoxSize &&
            ...
            : resizeObserverEntry.contentRect.height + this._editorHeightPadding
          : h;
    ...
    _setDefaultFormat: function (formatName) {
      ...
      if (fileComponent && !util.isTable(fileComponent)) {
        ...
        commonCon.nodeType === 1 &&
        commonCon.getAttribute("data-se-embed") === "true"
      ) {
        let el = commonCon.nextElementSibling;
        if (!util.isFormatElement(el))
  ...
  const event = {
    ...
    onMouseDown_wysiwyg: function (e) {
      if (core.isReadOnly || util.isNonEditable(context.element.wysiwyg))
        return;
        ...
        if (siblingNode && !util.isComponent(siblingNode))
          siblingNode =
            siblingKey === "previousSibling"
              ? siblingNode.firstChild
              : siblingNode.lastChild;
        else return null;
      }
    ...
    onKeyDown_wysiwyg: function (e) {
      ...
      switch (keyCode) {
        case 8 /** backspace key */:
          ...
          if (
            ...
            !util.isComponent(selectionNode) &&
            core._setDefaultFormat(options.defaultTag) !== null
          ) {
            e.preventDefault();
            e.stopPropagation();
            return false;
          ...
          if (
            !selectRange &&
            core._isEdgeFormat(range.startContainer, range.startOffset, "start")
          ) {
            if (util.isFormatElement(formatEl.previousElementSibling)) {
              core._formatAttrsTemp =
                formatEl.previousElementSibling.attributes;
            }
          }
          ...
          if (
            ...
            if (
              sel &&
              !sel.previousSibling &&
              ((commonCon && util.isComponent(commonCon.previousSibling)) ||
        ...
        case 46 /** delete key */:
          ...
          if (
            ...
            const sel =
              selectionNode === formatEl
                ? formatEl.childNodes[range.startOffset] || selectionNode
                : selectionNode;
            // delete nonEditable
          ...
          if (
            !selectRange &&
            core._isEdgeFormat(range.endContainer, range.endOffset, "end")
          ) {
        ...
        case 13 /** enter key */:
          ...
          if (!shift) {
            const formatEndEdge = core._isEdgeFormat(
              range.endContainer,
              range.endOffset,
              "end"
            );
            const formatStartEdge = core._isEdgeFormat(
              range.startContainer,
              range.startOffset,
              "start"
            );
            ...
            if (
              formatEndEdge &&
              (/^H[1-6]$/i.test(formatEl.nodeName) ||
              ...
              if (
                ...
                !util.isListCell(formatEl.nextElementSibling)
              ) {
                e.preventDefault();
                let newEl = null;
                  ...
                  const newFormat = util.isCell(rangeEl.parentNode)
                    ...
                    : util.isList(rangeEl.parentNode)
                      ...
                      : util.isFormatElement(rangeEl.nextElementSibling) &&
                          !util.isRangeFormatElement(rangeEl.nextElementSibling)
                        ? rangeEl.nextElementSibling.nodeName
                        : util.isFormatElement(
                              rangeEl.previousElementSibling
                            ) &&
                            !util.isRangeFormatElement(
                              rangeEl.previousElementSibling
                            )
              ...
              if (
                ...
                const newEl = core.appendFormatTag(
                  ...
                  util.isFormatElement(freeFormatEl.nextElementSibling) &&
                    !util.isRangeFormatElement(freeFormatEl.nextElementSibling)
            ...
            if (range.collapsed && (formatStartEdge || formatEndEdge)) {
              ...
              do {
                ...
                formatEl !== selectionNode &&
                formatEl.contains(selectionNode)
              );

              newFormat.appendChild(child);
              formatEl.parentNode.insertBefore(
                newFormat,
                formatStartEdge && !formatEndEdge
                  ? formatEl
                  : formatEl.nextElementSibling
              );
              if (formatEndEdge) {
            ...
            if (formatEl) {
              ...
              if (!range.collapsed) {
                ...
                newEl = newEl.contains(innerRange)
                  ? util.getChildElement(
                      innerRange,
                      util.getFormatElement.bind(util)
                    )
                  : newEl;
                if (isMultiLine) {
                  if (formatEndEdge && !formatStartEdge) {
                    newEl.parentNode.insertBefore(
                      newFormat,
                      !r.prevContainer || r.container === r.prevContainer
                        ? newEl.nextElementSibling
                        : newEl
                    );
                    newEl = newFormat;
                    offset = 0;
                    ...
                    offset = r.offset;
                    if (formatStartEdge) {
                      const tempEl = newEl.parentNode.insertBefore(
                        newFormat,
                        newEl
                      );
                      if (formatEndEdge) {
                  ...
                  if (formatEndEdge && formatStartEdge) {
                    ...
                    newEl = util.splitElement(
                      r.container,
                      r.offset,
                      util.getElementDepth(formatEl)
                    );
                  }
                }
              } else {
                if (util.onlyZeroWidthSpace(formatEl)) {
                  newEl = core.appendFormatTag(
                    formatEl,
                    formatEl.cloneNode(false)
                  );
                } else {
                  newEl = util.splitElement(
                    range.endContainer,
            ...
            if (util.isListCell(container.parentNode)) {
              ...
              newEl = util.createElement(
                util.isFormatElement(sibling) &&
                  !util.isRangeFormatElement(sibling)
                  ? sibling.nodeName
                  : options.defaultTag
              );
              newEl.innerHTML = "<br>";
      ...
      if (
        ...
        !alt &&
        !selectRange &&
        !event._nonTextKeyCode.test(keyCode) &&
    ...
    _resize_editor: function (e) {
      ...
      context.element.wysiwygFrame.style.height =
        context.element.code.style.height = h + "px";
      core._variable.resizeClientY = e.clientY;
    ...
    _hardDelete: function () {
      ...
      if (
        ...
        if (!sIsCell) {
          util.removeItem(
            util.getParentElement(eCell, function (current) {
              return ancestor === current.parentNode;
            })
          );
        } else if (!eIsCell) {
          util.removeItem(
            util.getParentElement(sCell, function (current) {
              return ancestor === current.parentNode;
            })
          );
        } else {
    ...
    onCopy_wysiwyg: function (e) {
      ...
      if (
        ...
        functions.onCopy(e, clipboardData, core) === false
      ) {
        e.preventDefault();
        e.stopPropagation();
        return false;
    ...
    onCut_wysiwyg: function (e) {
      ...
      if (
        ...
        functions.onCut(e, clipboardData, core) === false
      ) {
        e.preventDefault();
        e.stopPropagation();
        return false;
    ...
    _setClipboardData: function (type, e, plainText, cleanData, data) {
      ...
      if (!onlyText) {
        ...
        cleanData = core.cleanHTML(
          cleanData,
          core.pasteTagsWhitelistRegExp,
          core.pasteTagsBlacklistRegExp
        );
      } else {
  ...
  const functions = {
    ...
    setToolbarButtons: function (buttonList) {
      ...
      if (core.isReadOnly)
        util.setDisabledButtons(true, core.resizingDisabledButtons);
